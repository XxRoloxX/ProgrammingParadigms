...
module Queue_cyclic:QUEUE_MUT = struct
  type 'a t = {arr: 'a option array; mutable read: int;mutable write: int}
...
let empty initial_size = {arr=Array.make (initial_size+1) None; read=0; write=0}

let isEmpty queue = queue.read = queue.write

let isFull queue = if queue.read >0 then if queue.write =queue.read-1 then true else false else if queue.write=((Array.length queue.arr)-1) then true else false

let enqueue (new_element,queue) = match isFull queue with
false-> queue.arr.(queue.write)<-Some(new_element); queue.write<-((queue.write +1) mod Array.length queue.arr)
|true-> raise (Full "queue is full")

let dequeue (queue) = match isEmpty queue with
false->queue.read<-((queue.read +1) mod Array.length queue.arr)
|true -> raise (Empty "queue is empty")

let first (queue) = match isEmpty queue with
false-> let Some(result) = queue.arr.(queue.read) in result
|true->raise (Empty "queue is empty")
