...
def initSegment[A](list1:List[A],list2:List[A]):Boolean = (list1,list2) match
  case (List(),_) => true
  case (h::t,List()) => false
  case (h1::t1,h2::t2) if h1==h2 =>initSegment(list1.tail,list2.tail)
  case _ => false

initSegment(List(9,9),List())
...
def replaceNth[A](list:List[A],n:Int,element:A):List[A] = {
  def replaceNthAccum(list: List[A], n: Int, accum: List[A]):List[A] = list match
    case List()=> accum
    case h::t if n==0 => reverse_order(t):::(element::accum)
    case h::t => replaceNthAccum(list.tail, n-1, h::accum)
  reverse_order(replaceNthAccum(list,n,List()))
}

replaceNth(List(1),2,99)
